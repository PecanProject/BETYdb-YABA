...
def insert_experiments(username, fileName, status=True):
    ...
    try:
        ...
        if all(x in columns for x in accepted_columns):
            if status == False:
                msg = {"Message": "Successfully Validated", "Table": "Experiments"}
            ...
            msg = {
                "Message": "File not acceptable. Check the format of file or columns",
                "Table": "Experiments",
            }
...
def insert_sites(fileName, shp_file, dbf_file, prj_file, shx_file, status=True):
    ...
    try:
        ...
        if all(x in columns for x in accepted_columns):
            if status == False:
                msg = {"Message": "Successfully Validated", "Table": "Sites"}
...
def insert_treatments(username, fileName, status=True):
    ...
    try:
        ...
        if all(x in columns for x in accepted_columns):
            if status == False:
                msg = {"Message": "Successfully Validated", "Table": "Treatments"}
...
def insert_cultivars(fileName, status=True):
    ...
    try:
        ...
        if all(x in columns for x in accepted_columns):
            if status == False:
                msg = {"Message": "Successfully Validated", "Table": "Cultivars"}

                return make_response(jsonify(msg), 200)

            new_data = pd.DataFrame(
                columns=["name", "specie_id", "name", "ecotype", "notes"]
            )
...
def insert_citations(username, fileName, status=True):
    ...
    try:
        ...
        logging.error(traceback.format_exc())
        msg = {
            "Message": "(psycopg2.errors.UniqueViolation) duplicate key value violation."
        }
        return make_response(jsonify(msg), 409)
    except Exception as e:
        # Logs the error appropriately
...
def insert_experimentSites(fileName, status=True):
    ...
    try:
        ...
        if all(x in columns for x in accepted_columns):
            ...
            new_data["experiment_id"] = data.apply(
                lambda row: fetch_id(row["experiment_name"], table="experiments"),
                axis=1,
            )
            new_data["site_id"] = data.apply(
                lambda row: fetch_sites_id(row["sitename"]), axis=1
            )
...
def insert_experimentTreatments(fileName, status=True):
    ...
    try:
        ...
        if all(x in columns for x in accepted_columns):
            ...
            new_data["treatment_id"] = data.apply(
                lambda row: fetch_id(row["treatment_name"], table="treatments"), axis=1
            )
...
def insert_sitesCultivars(fileName, status=True):
    ...
    try:
        ...
        if all(x in columns for x in accepted_columns):
            if status == False:
                msg = {"Message": "Successfully Validated", "Table": "Sites_cultivars"}
            ...
            new_data["cultivar_id"] = data.apply(
                lambda row: fetch_cultivars_id(row["cultivar_name"], row["specie_id"]),
                axis=1,
            )
...
def insert_citationsSites(fileName, status=True):
    ...
    try:
        ...
        if all(x in columns for x in accepted_columns):
            if status == False:
                msg = {"Message": "Successfully Validated", "Table": "Citations_sites"}
            ...
            new_data["citation_id"] = data.apply(
                lambda row: fetch_citations_id(
                    row["author"], row["year"], row["title"]
                ),
                axis=1,
            )
            ...
            msg = {
                "Message": "File not acceptable and Check the format of file or columns",
