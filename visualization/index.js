...
let shp_upload = upload.fields([{ name: "shp_file", maxCount: 1 }]);

let api_key = upload.fields([{ name: "api_key", maxCount: 1 }]);

let parseCsv = (file) => {
  let buf = Buffer.from(file.buffer);
  let txt = buf.toString();
  let res = Papa.parse(txt)
    .data.slice(1)
    .map((item, idx) => {
      if (item.length > 1) return item;
      return null;
    });
  if (res.includes(null)) res.pop();
  return res;
};
...
app.post("/getCultivarSites", upload1, async (req, res) => {
  ...
  try {
    await getCultivarSites(
      cultivars,
      sites_cultivars,
      sites,
      geoJSON.features
    ).then((value) => getval(value));
    console.log(val);
